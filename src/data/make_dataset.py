...
# Convert 'epoch (ms)' column to datetime format
pd.to_datetime(df["epoch (ms)"], unit="ms")

# Set the index to the datetime format for both dataframes
acc_df.index = pd.to_datetime(acc_df["epoch (ms)"], unit="ms")
gyr_df.index = pd.to_datetime(gyr_df["epoch (ms)"], unit="ms")

# Delete unnecessary columns from accelerometer dataframe
del acc_df["epoch (ms)"]
del acc_df["time (01:00)"]
del acc_df["elapsed (s)"]

# Delete unnecessary columns from gyroscope dataframe
del gyr_df["epoch (ms)"]
