...
column = "acc_y"
LowPass.low_pass_filter(data_table=bench_set, col=column, sampling_frequency=fs, cutoff_frequency=0.4, order=10)
bench_set[column + "_lowpass"].plot()
...
# Group the DataFrame by 'label' and 'category', then calculate the mean of 'reps' and 'reps_pred'
rep_df.groupby(['label', 'category'])[['reps', 'reps_pred']].mean().plot.bar()
